...
int main( int argc, char *argv[] )
{
    ...
    double * u = calloc( sizeof(double), np*np );
    double * uhelp = calloc( sizeof(double), np*np );
    if( (!u) || (!uhelp) )
    {
    ...
    MPI_Recv(&u[(first_row-1)*np], (rows+2)*np, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
    MPI_Recv(&uhelp[(first_row-1)*np], (rows+2)*np, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, &status);
	...
	switch( algorithm ) {
	    case 0: // JACOBI
	            residual = relax_jacobi(&u[(first_row-1)*np], &uhelp[(first_row-1)*np], rows+2, np);
		    if (myid < (numprocs-1)) {
			MPI_Sendrecv(&uhelp[last_row*np], np, MPI_DOUBLE, myid+1, 0,
				     &uhelp[(last_row+1)*np], np, MPI_DOUBLE, myid+1, 0,
			   	     MPI_COMM_WORLD, &status);
			}
	 	    MPI_Sendrecv(&uhelp[first_row*np], np, MPI_DOUBLE, myid-1, 0,
		     		 &uhelp[(first_row-1)*np], np, MPI_DOUBLE, myid-1, 0,
		     		 MPI_COMM_WORLD, &status);
		    // Copy uhelp into u
		    for (int i=first_row-1; i<last_row+2; i++)
    		        for (int j=0; j<np; j++)
    ...
    MPI_Send(&u[first_row*np], rows*np, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
