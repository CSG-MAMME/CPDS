...
int main( int argc, char *argv[] ) {
    ...
    // CSG: First we need to define a pointer in CPU and pass it to GPU.
    float *dev_u, *dev_uhelp;
    cudaMalloc(&dev_u, sizeof(float) * np * np);
    cudaMalloc(&dev_uhelp, sizeof(float) * np * np);
    // CSG: Then we copy the values
    cudaMemcpy(dev_u, param.u, sizeof(float) * np * np,
               cudaMemcpyHostToDevice);
    cudaMemcpy(dev_uhelp, param.uhelp, sizeof(float) * np * np,
               cudaMemcpyHostToDevice);

    // CSG: Additional values for the reduction
    float *res_in, *res_out, *dev_res_in, *dev_res_out;
    res_in = (float *) malloc(sizeof(float) * np * np);
    res_out = (float *) malloc(sizeof(float) * np);
    cudaMalloc(&dev_res_in, sizeof(float) * np * np);
    cudaMalloc(&dev_res_out, sizeof(float) * np);

    iter = 0;
    while(1) {
        // CSG: If we don't do the reduction, we don't need to 
        // keep track of the residuals. To implement the reduction
        // we must modify the function prototype.
        // gpu_Heat<<<Grid,Block>>>(dev_u, dev_uhelp, np);
        gpu_Heat<<<Grid,Block>>>(dev_u, dev_uhelp, dev_res_in, np);
        cudaThreadSynchronize();                        // wait for all threads to complete

        // CSG: Residual computed in CPU
        // cudaMemcpy(param.u, dev_u, sizeof(float) * np * np,
        //            cudaMemcpyDeviceToHost);
        // cudaMemcpy(param.uhelp, dev_uhelp, sizeof(float) * np * np,
        //            cudaMemcpyDeviceToHost);
        // residual = cpu_residual (param.u, param.uhelp, np, np);

        // CSG: Alternatively do a reduction on the residual.
        cudaMemcpy(res_in, dev_res_out, sizeof(float) * np,
                   cudaMemcpyDeviceToHost);
        gpu_Reduction<<<np, np, sizeof(float) * np * np>>>(dev_res_in,
                                                           dev_res_out);
        cudaThreadSynchronize();
        cudaMemcpy(res_out, dev_res_out, sizeof(float) * np,
                   cudaMemcpyDeviceToHost);
        ...
        // CSG: If we reduce using a Kernel, the result is a np-sized
        // vector, we need to do the final step of the reduction by hand.
        // Alternatively, we could implement another kernel?
        residual = 0;
        for (unsigned int i = 0; i < np; i++) residual += res_out[i];
    ...
    // CSG: Similarly to before
    cudaMemcpy(param.u, dev_u, sizeof(float) * np * np,
               cudaMemcpyDeviceToHost);

    // CSG: free memory used in GPU
    cudaFree(dev_u);
    cudaFree(dev_uhelp);
    // CSG: free additional structures used in reduce
    cudaFree(dev_res_in);
    cudaFree(dev_res_out);
